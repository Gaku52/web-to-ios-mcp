...
import { AnyProjectInfo } from '../types/index.js';

export async function detectFrameworkTool(args: any) {
  ...
  try {
    ...
    const projectInfo: AnyProjectInfo | null = viteProject || nextjsProject;
 ...
 */
function isCapacitorReady(projectInfo: AnyProjectInfo): boolean {
  if (projectInfo.framework === 'nextjs') {
